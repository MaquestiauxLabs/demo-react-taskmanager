...
describe("Roles API integration", () => {
  ...
  it("GET /api/roles returns seeded roles", async () => {
  ...
  it("POST /api/roles creates a role", async () => {
    const name = `IntegrationRole${Date.now()}`;
  ...
  it("POST /api/roles returns 404 when creator does not exist", async () => {
  ...
  it("GET /api/roles/:id returns 404 for unknown role", async () => {
    const response = await request(app).get("/api/roles/not-a-real-id");
  ...
  it("PUT /api/roles/:id updates an existing role", async () => {
    ...
    const updated = await request(app)
      .put(`/api/roles/${created.body.data.id}`)
      .send({ name: `Updated${Date.now()}`, color: "#00aa00" });

    expect(updated.status).toBe(200);
    expect(updated.body.message).toContain("Update role with ID");
    expect(updated.body.data.color).toBe("#00aa00");
  });

  it("DELETE /api/roles/:id deletes an existing role", async () => {
    const created = await request(app)
      .post("/api/roles")
      .send({
    ...
    const deleted = await request(app).delete(
      `/api/roles/${created.body.data.id}`,
    );

    expect(deleted.status).toBe(200);
    expect(deleted.body.message).toContain("Delete role with ID");
  });
});
